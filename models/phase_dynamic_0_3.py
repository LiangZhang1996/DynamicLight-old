"""
DynamicLight under feature fusion method 3
Input shape: [batch, max_lane*4]
Created by Liang Zhang
"""
from tensorflow.keras.layers import Input, Dense, Reshape,  Lambda,  Subtract, Add
...
class DynamicAgent03(NetworkAgent):
    ...
    def build_network(self):
        ...
        for i in range(self.num_phases):
            embed_to_same_dim.append(Dense(20, activation="relu"))

        phase_feats_map_2 = []
        for i in range(self.num_phases):
            tmp_feat_1 = tf.concat([lane_feats_s[idx] for idx in self.phase_map[i]], axis=-1)
            phase_feats_map_2.append(embed_to_same_dim[i](tmp_feat_1))
        ...
        selected_phase_feat = Reshape((20, ))(selected_phase_feat)
